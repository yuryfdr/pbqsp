...
void ParseChildren(PBListBox & listBox,
		   ...
		   tree < htmlcxx::HTML::Node >::iterator & itp,
		   insert_op & iop)
{
  ...
  if (it->isTag()) {
    ...
    if (it->tagName() == "a" || it->tagName() == "A") {
      ...
      if(it.begin()!=it.end())
        links.push_back(
          make_pair(std::string("[")+to_utf8(it.begin()->text().c_str(),koi8_to_unicode)+std::string("]"),
          it->attribute("href").second) );
      else
        ciop.to_parent = true;
      ...
      PBListBoxItem *newItem = listBox.addItem(image, ciop.tag, ciop.align);
      if (!ciop.is_link)
    ...
    } else if (it->tagName() == "h2" || it->tagName() == "H2"
            || it->tagName() == "h1" || it->tagName() == "H1") {
      ciop.font = biggerFont;
    } else if (it->tagName() == "h3" || it->tagName() == "H3"
            || it->tagName() == "h4" || it->tagName() == "H4") {
      ciop.font = boldFont;
      ...
      if (sz.size() == 1) {
        if (size > 3) ciop.font = biggerFont;
      } else {
        if (size > 0) ciop.font = biggerFont;
      }
    }
  } else {
...
#endif
        PBWidget* ctl = new PBWidget(to_utf8(it->text().c_str(), koi8_to_unicode),itm);
...
#endif
          PBListBoxItem *newItem = listBox.addItem(to_utf8(it->text().c_str(), koi8_to_unicode),
                                                 ciop.tag, ciop.align);
          if (!ciop.is_link) newItem->setCanBeFocused(false);
          if (ciop.font != defaultFont) newItem->setWidgetFont(ciop.font);
          ciop.item = newItem;
          ciop.to_parent=false;
          ciop.is_link=false;
        }
		...
		std::string > >&links)
{
  ...
  if (src_text == 0 || strlen(src_text) == 0) return;

#ifdef HTMLDEBUG
  std::cerr << to_utf8(src_text, koi8_to_unicode) << std::endl;
#endif
...
void SetStringToCharString(char *dest, const std::string & src,
			   int dest_size)
{
