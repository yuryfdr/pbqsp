...
void* playfn(void* pf){
  if(played.name.size()!=0 && played.name.size()>4){
    ...
    std::string name=(to_utf8(played.name.substr(GetQuestPath().size()).c_str(), koi8_to_unicode));
    name=GetQuestPath()+name;
    std::string runstr;
    //const char* args[narg+3];
    const char* const * targs;
    if(ext==".mp3"){
      runstr="mpg123";
      //mpg123[narg]=convertbackslash(name).c_str();
      targs=mpg123;
    ...
    if(played.pid!=-1)kill(played.pid,SIGKILL);
    pid_t pid = fork();
    played.played=true;
    pthread_mutex_unlock(&played.mt);
...
void QSPCallbacks::CloseFile(const QSP_CHAR *file)
{
  ...
  pthread_mutex_unlock(&played.mt);
  if(played.th!=-1)pthread_join(played.th,&ret);
  fprintf(stderr,"after join %d\n",played.th);
  played.th=-1;
  /*pthread_mutex_lock(&played.mt);
  pthread_cancel(played.th);
  pthread_mutex_unlock(&played.mt);*/
}
